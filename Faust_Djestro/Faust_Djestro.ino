// PÚCA_DSP - Faust dsp karplus-strong + djembe //
...
djestro* DSP = new djestro(SAMPLE_RATE, NO_SAMPLES);

float freq, damp, posit, sharp, gain, gate;

void update_parameters (void *pvParameters) {
  for (;;) {
    for (uint32_t i = 0; i < NO_SAMPLES; i++) {
      DSP->setParamValue("freq", freq);
      DSP->setParamValue("damp", damp);
      DSP->setParamValue("posit", posit);
...
void loop() {
  ...
  damp = randomf(0.001f, 0.1f);
  freq = randomf(0.01f, 0.5f);
  posit = randomf(0.1f, 0.9f);
  sharp = randomf(0.1f, 0.9f);
  gate = 1.0f;
