  ...
  public:
	...
	void fillmydspSIG0(int count, float* table) {
		for (int i1 = 0; i1 < count; i1 = i1 + 1) {
			iVec1[0] = 1;
			iRec3[0] = (iVec1[1] + iRec3[1]) % 256;
			table[i1] = std::cos(0.024543693f * float(iRec3[0]));
			iVec1[1] = iVec1[0];
			iRec3[1] = iRec3[0];
		}
	}
  ...
  public:
	...
	void fillmydspSIG1(int count, float* table) {
		for (int i2 = 0; i2 < count; i2 = i2 + 1) {
			iVec3[0] = 1;
			iRec7[0] = (iVec3[1] + iRec7[1]) % 256;
			table[i2] = std::sin(0.024543693f * float(iRec7[0]));
			iVec3[1] = iVec3[0];
			iRec7[1] = iRec7[0];
		}
	}
...
static float ftbl1mydspSIG1[256];
...
class mydsp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		m->declare("author", "Critter&Guitari");
	...
	static void classInit(int sample_rate) {
		...
		sig1->fillmydspSIG1(256, ftbl1mydspSIG1);
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("freq", &fHslider0, FAUSTFLOAT(4.4e+02f), FAUSTFLOAT(2e+01f), FAUSTFLOAT(2e+03f), FAUSTFLOAT(0.01f));
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow1 = 0.0f - 1.9995203f * std::cos(0.00026179937f * fSlow0);
		...
		float fSlow7 = 0.0f - 1.9904289f * std::cos(0.00013089969f * (fSlow6 + 3.8e+03f));
		float fSlow8 = 0.0f - 1.9952087f * std::cos(0.00013089969f * (fSlow6 + 3.6e+03f));
		float fSlow9 = 0.0f - 1.9968045f * std::cos(0.00013089969f * (fSlow6 + 3.4e+03f));
		float fSlow10 = 0.0f - 1.9976029f * std::cos(0.00013089969f * (fSlow6 + 3.2e+03f));
		float fSlow11 = 0.0f - 1.998082f * std::cos(0.00013089969f * (fSlow6 + 3e+03f));
		float fSlow12 = 0.0f - 1.9984016f * std::cos(0.00013089969f * (fSlow6 + 2.8e+03f));
		float fSlow13 = 0.0f - 1.9986299f * std::cos(0.00013089969f * (fSlow6 + 2.6e+03f));
		float fSlow14 = 0.0f - 1.9988011f * std::cos(0.00013089969f * (fSlow6 + 2.4e+03f));
		float fSlow15 = 0.0f - 1.9989343f * std::cos(0.00013089969f * (fSlow6 + 2.2e+03f));
		float fSlow16 = 0.0f - 1.9990408f * std::cos(0.00013089969f * (fSlow6 + 2e+03f));
		float fSlow17 = 0.0f - 1.999128f * std::cos(0.00013089969f * (fSlow6 + 1.8e+03f));
		float fSlow18 = 0.0f - 1.9992007f * std::cos(0.00013089969f * (fSlow6 + 1.6e+03f));
		float fSlow19 = 0.0f - 1.9992621f * std::cos(0.00013089969f * (fSlow6 + 1.4e+03f));
		float fSlow20 = 0.0f - 1.9993148f * std::cos(0.00013089969f * (fSlow6 + 1.2e+03f));
		float fSlow21 = 0.0f - 1.9993604f * std::cos(0.00013089969f * (fSlow6 + 1e+03f));
		float fSlow22 = 0.0f - 1.9994005f * std::cos(0.00013089969f * (fSlow6 + 8e+02f));
		float fSlow23 = 0.0f - 1.9994357f * std::cos(0.00013089969f * (fSlow6 + 6e+02f));
		float fSlow24 = 0.0f - 1.999467f * std::cos(0.00013089969f * (fSlow6 + 4e+02f));
		float fSlow25 = 0.0f - 1.9994951f * std::cos(0.00013089969f * (fSlow6 + 2e+02f));
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			iVec0[0] = 1;
			iRec2[0] = 1103515245 * iRec2[1] + 12345;
			fRec1[0] = 0.5221894f * fRec1[3] + 4.656613e-10f * float(iRec2[0]) + 2.494956f * fRec1[1] - 2.0172658f * fRec1[2];
			float fTemp0 = 0.049922034f * fRec1[0] + 0.0506127f * fRec1[2] - (0.095993534f * fRec1[1] + 0.004408786f * fRec1[3]);
			iVec2[0] = iSlow2;
			int iTemp1 = iSlow2 - iVec2[1];
			iRec6[0] = iSlow2 * (iRec6[1] + 1);
			int iTemp2 = (iRec6[0] < 48) | (iTemp1 * (iTemp1 > 0));
			float fTemp3 = 0.1447178f * ((iSlow2) ? ((iTemp2) ? 0.001f : 0.005f) : 0.005f);
			int iTemp4 = std::fabs(fTemp3) < 1.1920929e-07f;
			float fTemp5 = ((iTemp4) ? 0.0f : std::exp(0.0f - 2.0833333e-05f / ((iTemp4) ? 1.0f : fTemp3)));
			float fTemp6 = ((iSlow2) ? ((iTemp2) ? fSlow3 : 0.0f) : 0.0f);
			fRec5[0] = (1.0f - fTemp5) * fTemp6 + fTemp5 * fRec5[1];
			float fTemp7 = ((1 - iVec0[1]) ? 0.0f : fRec4[1] + 2.0833333e-05f * (fSlow4 + 4e+03f * fRec5[0]));
			fRec4[0] = fTemp7 - std::floor(fTemp7);
			int iTemp8 = std::max<int>(0, std::min<int>(int(256.0f * fRec4[0]), 255));
			float fTemp9 = 0.1447178f * ((iSlow2) ? ((iTemp2) ? 0.001f : 0.07f) : 0.07f);
			int iTemp10 = std::fabs(fTemp9) < 1.1920929e-07f;
			float fTemp11 = ((iTemp10) ? 0.0f : std::exp(0.0f - 2.0833333e-05f / ((iTemp10) ? 1.0f : fTemp9)));
			fRec8[0] = fTemp6 * (1.0f - fTemp11) + fTemp11 * fRec8[1];
			float fTemp12 = fSlow5 * fRec8[0] * (ftbl1mydspSIG1[iTemp8] * std::cos(fTemp0) + ftbl0mydspSIG0[iTemp8] * std::sin(fTemp0));
			fRec0[0] = fTemp12 - (fSlow1 * fRec0[1] + 0.9995204f * fRec0[2]);
			fRec9[0] = fTemp12 - (fSlow7 * fRec9[1] + 0.99045175f * fRec9[2]);
			fRec10[0] = fTemp12 - (fSlow8 * fRec10[1] + 0.99521446f * fRec10[2]);
			fRec11[0] = fTemp12 - (fSlow9 * fRec11[1] + 0.9968071f * fRec11[2]);
			fRec12[0] = fTemp12 - (fSlow10 * fRec12[1] + 0.99760437f * fRec12[2]);
			fRec13[0] = fTemp12 - (fSlow11 * fRec13[1] + 0.998083f * fRec13[2]);
			fRec14[0] = fTemp12 - (fSlow12 * fRec14[1] + 0.99840224f * fRec14[2]);
			fRec15[0] = fTemp12 - (fSlow13 * fRec15[1] + 0.99863034f * fRec15[2]);
			fRec16[0] = fTemp12 - (fSlow14 * fRec16[1] + 0.99880147f * fRec16[2]);
			fRec17[0] = fTemp12 - (fSlow15 * fRec17[1] + 0.99893457f * fRec17[2]);
			fRec18[0] = fTemp12 - (fSlow16 * fRec18[1] + 0.999041f * fRec18[2]);
			fRec19[0] = fTemp12 - (fSlow17 * fRec19[1] + 0.99912816f * fRec19[2]);
			fRec20[0] = fTemp12 - (fSlow18 * fRec20[1] + 0.9992008f * fRec20[2]);
			fRec21[0] = fTemp12 - (fSlow19 * fRec21[1] + 0.9992623f * fRec21[2]);
			fRec22[0] = fTemp12 - (fSlow20 * fRec22[1] + 0.99931496f * fRec22[2]);
			fRec23[0] = fTemp12 - (fSlow21 * fRec23[1] + 0.9993606f * fRec23[2]);
			fRec24[0] = fTemp12 - (fSlow22 * fRec24[1] + 0.99940056f * fRec24[2]);
			fRec25[0] = fTemp12 - (fSlow23 * fRec25[1] + 0.9994358f * fRec25[2]);
			fRec26[0] = fTemp12 - (fSlow24 * fRec26[1] + 0.99946713f * fRec26[2]);
			fRec27[0] = fTemp12 - (fSlow25 * fRec27[1] + 0.99949515f * fRec27[2]);
			float fTemp13 = 0.0025f * (fRec0[0] + 0.25f * (fRec27[0] - fRec27[2]) + 0.11111111f * (fRec26[0] - fRec26[2]) + 0.0625f * (fRec25[0] - fRec25[2]) + 0.04f * (fRec24[0] - fRec24[2]) + 0.027777778f * (fRec23[0] - fRec23[2]) + 0.020408163f * (fRec22[0] - fRec22[2]) + 0.015625f * (fRec21[0] - fRec21[2]) + 0.012345679f * (fRec20[0] - fRec20[2]) + 0.01f * (fRec19[0] - fRec19[2]) + 0.008264462f * (fRec18[0] - fRec18[2]) + 0.0069444445f * (fRec17[0] - fRec17[2]) + 0.00591716f * (fRec16[0] - fRec16[2]) + 0.0051020407f * (fRec15[0] - fRec15[2]) + 0.0044444446f * (fRec14[0] - fRec14[2]) + 0.00390625f * (fRec13[0] - fRec13[2]) + 0.0034602077f * (fRec12[0] - fRec12[2]) + 0.0030864198f * (fRec11[0] - fRec11[2]) + 0.002770083f * (fRec10[0] - fRec10[2]) + 0.0025f * (fRec9[0] - fRec9[2]) - fRec0[2]);
...
marimba::marimba(int sample_rate, int buffer_size)
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void marimba::setParamValue(const string& path, float value)
{
    fUI->setParamValue(path, value);
}

float marimba::getParamValue(const string& path)
{
...
extern "C" void app_main()
{
    ...
    marimba* DSP = new marimba(48000, 32);
    DSP->start();
