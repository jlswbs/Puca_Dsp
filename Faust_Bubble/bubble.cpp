...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("shape", &fHslider0, FAUSTFLOAT(6e+02f), FAUSTFLOAT(1.5e+02f), FAUSTFLOAT(2e+03f), FAUSTFLOAT(1.0f));
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow3 = 1.5625e-06f * fSlow2;
		float fSlow4 = 2.0833333e-05f * fSlow1;
		float fSlow5 = 2.0833333e-05f * fSlow2;
		float fSlow6 = 0.5f * float(fHslider1);
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			fRec3[0] = 0.01f * std::exp(0.0f - fSlow5 * fTemp0) + 0.99f * fRec3[1];
			float fTemp2 = fSlow6 * fRec3[0] * ftbl0mydspSIG0[std::max<int>(0, std::min<int>(int(256.0f * fRec1[0]), 255))];
...
bubble::bubble(int sample_rate, int buffer_size)
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void bubble::setParamValue(const string& path, float value)
{
    fUI->setParamValue(path, value);
}

float bubble::getParamValue(const string& path)
{
...
extern "C" void app_main()
{
    ...
    bubble* DSP = new bubble(48000, 32);
    DSP->start();
