...
kick* DSP = new kick(SAMPLE_RATE, NO_SAMPLES);

float freq, att, dec, depth, attf, decf, gain, gate;
...
void update_parameters (void *pvParameters) {
  for (;;) {
    for (uint32_t i = 0; i < NO_SAMPLES; i++) {
      ...
      DSP->setParamValue("att", att);
      DSP->setParamValue("dec", dec);
      DSP->setParamValue("depth", depth);
      DSP->setParamValue("attf", att);
      DSP->setParamValue("decf", dec);
...
void loop() {
  ...
  dec = randomf(0.1f, 0.5f);
  freq = 55 + esp_random() % 220;
  gate = 1.0f;
